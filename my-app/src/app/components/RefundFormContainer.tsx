...
const RefundFormContainer = () => {
  ...
  const submitRefundForm=async(e: { preventDefault: () => void; })=>{
    ...
    if(imageFileArray.length!==0){
      imageFileArray.forEach((file,index)=>{
  ...
  return (
    <form className='w-full h-full flex flex-col gap-2' onSubmit={submitRefundForm}>
      <div className='flex flex-col w-2/3 gap-2'>
        <label htmlFor='delivery-options' className=''>Select Reason</label>
            <select id='reason-options' className='text-md font-semibold ms-5 rounded-md p-1'
      ...
      <input type="file" placeholder='+Add image' title='Add image'
      className='rounded-2xl p-1' onChange={handleImageInput}/>
      </div>

      <label htmlFor="">Provide Reason</label>
      <input type="text" placeholder='Provide reason for refund' 
...
<div className='w-full flex items-center justify-end text-black font-semibold'>
  <button type='submit' className='bg-pink-200 border border-pink-300 hover:bg-pink-300 p-2 rounded-lg w-max'>Submit Application</button>
</div>
