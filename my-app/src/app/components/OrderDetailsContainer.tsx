...
import { setShippingAddress,addShippingAddress,setPayments } from '@/store/userSlice';
...
<div className='gap-2 items-start flex w-full overflow-x-auto'>
      {shippingAddress.map((item, index) => (
        ...
        >
          <div className={`w-52 h-32 border-[2px] rounded-xl shadow-2xl flex flex-col cursor-pointer
            ...
            <div className='flex justify-between p-3'>
              ...
              <button><img src="/horizontal_dots.svg" alt="Options" /></button>
            </div>
      ...
      <div className='w-full rounded-2xl bg-white h-max shadow-xl p-5 flex flex-col gap-2'>
        ...
        <button className='text-pink-300 font-bold text-sm flex justify-start'
        onClick={()=>addPaymentContainerVisibilityFunction(true)}>+  Add Payment Method</button>

        <div className='flex gap-2 items-center'>
          {paymentMethods.map((item,index)=>(
             <div key={index} onClick={()=>handleSelectPayment(index)} className={`w-1/4 h-32 border-[2px] rounded-xl shadow-2xl 
            ...
            <div className='flex justify-between p-3'>
              <label className='text-black font-medium text-sm'>Address</label>
              <button><img src="/horizontal_dots.svg" alt="" /></button>
            </div>
          ...
          <div className='w-1/4 h-32 border-[2px] border-pink-300 bg-pink-100 rounded-xl shadow-2xl hover:w-1/3 hover:h-40 hidden'>
            <div className='flex justify-between p-3'>
              <label className='text-black font-medium text-sm'>Address</label>
              <button><img src="/horizontal_dots.svg" alt="" /></button>
            </div>
            <div className='flex flex-col items-center justify-center'>
              <img src="/visa_icon.svg" alt="" width={40} height={40} className='justify-center' />
              <p className='text-sm font-bold text-gray-600'>123456789012345</p>
            </div>
          </div>
...
{addPaymentContainerVisibility && (
  ...
  <select id='payment_type' className='h-8 rounded-md w-full'>
  ...
  <input type="text" id='accountNameInput' placeholder='Account Name' className='w-full rounded-md h-8 bg-white border border-pink-300 px-5 text-black' />

  <label htmlFor="accountNumberInput" className='text-black text-sm font-semibold'>Account Number</label>
  <input type="number" id='accountNumberInput' placeholder='Account Number' className='w-full rounded-md h-8 bg-white border border-pink-300 px-5 text-black' />

  <div className='flex gap-3'>
    <button className='font-bold text-black text-md border border-red-400 bg-red-300 w-full p-1 rounded-md hover:bg-red-400'
    onClick={()=>addPaymentContainerVisibilityFunction(false)}>Close</button>
    <button type='submit' className='font-bold text-black text-md border border-pink-300 bg-pink-200 w-full p-1 rounded-md hover:bg-pink-300'
    onClick={()=>addPaymentMethod()}>Add</button>
  </div>
</div>
)}
